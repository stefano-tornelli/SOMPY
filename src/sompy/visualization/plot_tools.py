...
def plot_hex_map(d_matrix, titles=[], colormap=cm.gray, shape=[1, 1], comp_width=5, hex_shrink=1.0, fig=None, colorbar=True):
    """
    ...
    def create_grid_coordinates(x, y):
        coordinates = [x for row in -1 * np.array(list(range(x))) for x in list(zip(np.arange(((row) % 2) * 0.5, y + ((row) % 2) * 0.5), [0.8660254 * (row)] * y))]
    ...
    if fig is None:
        ...
        fig = plt.figure(figsize=(xinch, yinch), dpi=72.0)
    ...
    for comp, title in zip(range(d_matrix.shape[2]), titles):
        ...
        ax.axis([min(xpoints) - 1.0, max(xpoints) + 1.0, min(ypoints) - 1.0, max(ypoints) + 1.0])
