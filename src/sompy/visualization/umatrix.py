...
class UMatrixView(MapView):
    def build_u_matrix(self, som, distance=1, row_normalized=False):
        ...
        for i in range(som.codebook.nnodes):
            ...
            neighborbor_dists = scipy.spatial.distance_matrix(codebook_i, neighborbor_codebooks)
    ...
    def _set_contour(self, umat, ax, X=None, Y=None, hex=False):
        ...
        if hex:
            ax.contour(X, Y, umat, np.linspace(mn, md, 15), linewidths=0.7, cmap=plt.cm.get_cmap("Blues"))
        else:
            ax.contour(umat, np.linspace(mn, md, 15), linewidths=0.7, cmap=plt.cm.get_cmap("Blues"))

    def _set_show_data(self, X, Y, ax):
        ax.scatter(X, Y, s=2, alpha=1.0, c="Gray", marker="o", cmap="jet", linewidths=3, edgecolor="Gray")

    def _set_labels(self, labels, X, Y, ax):
        for label, x, y in zip(labels, X, Y):
            ax.annotate(str(label), xy=(x, y), horizontalalignment="center", verticalalignment="center")

    def _set_blob(self, umat, coord, ax, X=None, Y=None, hex=False):
        # 'Laplacian of Gaussian'
        image = 1 / umat
        blobs = blob_log(image, max_sigma=5, num_sigma=4, threshold=0.152)
        ...
        for blob in blobs:
            row, col, r = blob
            c = plt.Circle((row, col), r, color="red", linewidth=2, fill=False)
            ax.add_patch(c)

            dist = scipy.spatial.distance_matrix(coord, np.array([row, col])[np.newaxis, :])
            sel_point = dist <= r
            ax.plot(coord[:, 0][sel_point[:, 0]], coord[:, 1][sel_point[:, 0]], ".r")
    ...
    def show(self, som, distance=1, row_normalized=False, show_data=False, contour=False, blob=False, labels=False):
        # Setting figure parameters
        org_w = self.width
        org_h = self.height
        (self.width, self.height, indtoshow, no_row_in_plot, no_col_in_plot, axis_num) = self._calculate_figure_params(som, 1, 1)
        ...
        if som.codebook.lattice == "rect":
            ...
            raise ValueError('lattice argument of SOM object should be either "rect" or "hexa".')
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.90, hspace=0.00, wspace=0.000)
