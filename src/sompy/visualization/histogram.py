...
class Hist2d(MatplotView):
    ...
    def _fill_hist(self, x, y, mapsize, data_coords, what="train"):
        x = np.arange(0.5, mapsize[1] + 0.5, 1)
        y = np.arange(0.5, mapsize[0] + 0.5, 1)
        X, Y = np.meshgrid(x, y)

        if what == "train":
            a = plt.hist2d(x, y, bins=(mapsize[1], mapsize[0]), alpha=0.0, cmap=cm.jet)
            area = a[0].T * 12
            plt.scatter(data_coords[:, 1], mapsize[0] - 0.5 - data_coords[:, 0], s=area.flatten(), alpha=0.9, c="None", marker="o", cmap="jet", linewidths=3, edgecolor="r")

        else:
            a = plt.hist2d(x, y, bins=(mapsize[1], mapsize[0]), alpha=0.0, cmap=cm.jet, norm=LogNorm())
            area = a[0].T * 50
            plt.scatter(data_coords[:, 1] + 0.5, mapsize[0] - 0.5 - data_coords[:, 0], s=area, alpha=0.9, c="None", marker="o", cmap="jet", linewidths=3, edgecolor="r")
            plt.scatter(data_coords[:, 1] + 0.5, mapsize[0] - 0.5 - data_coords[:, 0], s=area, alpha=0.2, c="b", marker="o", cmap="jet", linewidths=3, edgecolor="r")
